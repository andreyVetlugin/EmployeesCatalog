...
namespace EmployeesCatalog.Web.controllers
{
    public class ProfilesController : BaseApiController
    {
        ...
        {
            var mapConfig = new MapperConfiguration(cfg => cfg.CreateMap<Profile, ProfileShortView>());
            var mapper = new Mapper(mapConfig);
            var resultFromHandler = requestHandler.GetRange(startIndex,itemsCount);

            return ApiResultGenerator.GenerateResult
                (resultFromHandler.BuildWithChangeData(l => new ViewListElements<ProfileShortView>
                { Items = mapper.Map<List<ProfileShortView>>(l.items), ItemsCount = l.itemsCount }));
        }
